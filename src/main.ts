...
async function bootstrap() {
  ...
  const config = new DocumentBuilder()
    .setTitle('Doctor API') // Set the API title
    .setDescription('Doctor App') // Set the API description
    .setVersion('1.0') // Set the API version
    .addTag('doctor api platform') // Add a tag to categorize API endpoints
    .build();
